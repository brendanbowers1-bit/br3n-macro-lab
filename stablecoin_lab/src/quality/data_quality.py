"""
Strict Data Quality Score (0–100) for Stablecoin Settlement Window Lab.
"""
...
def score_row(row: pd.Series, required_fields: list[str] | None = None) -> dict:
    ...
    if row.get("date_granularity") in ("daily", "hourly", "transaction"):
        pts += RUBRIC_WEIGHTS["granularity"]
    elif row.get("blockchain_network") or row.get("stablecoin"):
        pts += 6

    supply = float(row.get("supply_usd", row.get("total_reserves_usd", 1)) or 0)
    peg = float(row.get("peg_deviation_bps", 0) or 0)
    if supply >= 0 and abs(peg) < 5000:
