# FX Desk Risk Memo

**Corridor:** US_CO / USD/COP

*Research and risk-framing only. Not investment advice. Not a trading instruction.*

## Market State

- **Latest regime:** R2_trend_low_vol
- **Market summary:** Regime: R2_trend_low_vol · Volatility: Low volatility
- **Crisis risk level:** Low
- **Overall desk risk:** Moderate
- **Data quality:** Data quality flag: OK. Prototype data — upgrade for publication.

## Key Desk Questions

### 1. Exposure
What exposure needs to be confirmed by currency, value date, entity, and counterparty?

### 2. Hedge Timing
Orderly trend. Consider gradual hedge adjustment within policy limits.

### 3. Pricing
**Customer pricing posture:** Normal with trend awareness — monitor hedge cost pass-through.

### 4. Liquidity and Prefunding
**Liquidity:** Standard — routine liquidity monitoring.

**Prefunding:** Standard prefunding discipline unless corridor data warns otherwise.

### 5. Settlement
Standard settlement calendar review.

### 6. Crisis / Escalation
Crisis risk level: **Low**. Escalate if liquidity, spreads, or payout obligations deteriorate beyond policy.

### 7. Speculation Control
All hedge actions must map to documented exposure and policy limits. Model signals are not directional trading instructions.

## Summary

Overall desk risk level is Moderate. Hedge timing posture: Orderly trend. Forecast evidence is weak, but regime information may still be useful for hedge governance and risk framing.

## Disclaimer

Bowers Frontier Macro Labs is an independent research project. It is not affiliated with, endorsed by, or sponsored by any employer, financial institution, payment company, trading platform, or data vendor. Research and risk-framing only. Not investment advice. Not a trading instruction. Requires human review, exposure systems, and policy controls.