...
CORRIDOR_JSON = ROOT / "data" / "outputs" / "us_mx_corridor_daily.json"
...
<main class="bfi-main">
  ...
  <section class="bfi-section">
    <h2>Bowers Frontier Macro Labs</h2>
    ...
    <p><a href="macro-lab.html" class="bfi-cta secondary">Enter Bowers Frontier Macro Labs →</a></p>
  </section>
</main>
"""
...
<section class="bfi-hero" style="padding-bottom:2rem;">
  ...
  <h1>Dashboards</h1>
  <p class="lead">Interactive research interfaces built by Bowers Frontier Studio. Python computes; dashboards visualize. Research only — not investment advice.</p>
</section>
<main class="bfi-main">
  <section class="bfi-section">
    {_card_grid([
...
def _corridor_payload() -> dict:
    if not CORRIDOR_JSON.exists():
        return {}
    return json.loads(CORRIDOR_JSON.read_text(encoding="utf-8"))


def bfi_us_mexico_corridor_body() -> str:
    p = _corridor_payload()
    kpis = p.get("kpis", {})
    crs = p.get("risk_score", {})
    validated = p.get("validation", {}).get("passed", False)
    data_label = "validated pipeline" if validated else "starter data"
    crs_score = crs.get("score", "—")
    crs_band = crs.get("band", "—")
    latest = kpis.get("latest_flow", 5060)
    yoy_pct = kpis.get("yoy_pct", -7.8)
    ytd = kpis.get("ytd_2025", 55651)
    peak = kpis.get("peak_flow", 5723)
    yoy_class = "negative" if yoy_pct < 0 else "positive" if yoy_pct > 0 else "neutral"
    yoy_sign = "+" if yoy_pct >= 0 else ""
    yoy_abs = kpis.get("yoy_abs")
    yoy_abs_text = f"{yoy_sign}{yoy_abs:,.0f}M vs prior year month" if yoy_abs is not None else "YoY reference unavailable"
    brief_link = ""
    if (ROOT / "reports" / "publication" / "us-mx-corridor-brief.md").exists():
        brief_link = '<p class="page-disclaimer"><a href="us-mx-corridor-brief.md">Latest corridor intelligence brief</a> (markdown, research only)</p>'

    return f"""
<main class="bfi-corridor-page-main">
  <section class="page-hero light-zone">
    <div class="bfi-page-inner">
      <div class="page-eyebrow">USD/MXN CORRIDOR INTELLIGENCE</div>
      <h1>United States → Mexico corridor.</h1>
      <p class="page-subtitle">Validated remittance flows, transparent corridor risk score, and research briefs for treasury decision-support.</p>
      <p class="page-thesis">The US/Mexico corridor connects household income, labor markets, exchange rates, settlement rails, and cross-border financial infrastructure. This page surfaces structural flow stress — not FX predictions.</p>
  ...
  <section class="corridor-dashboard dark-zone">
    <div class="bfi-page-inner">
      ...
      <h2 class="dark-section-title">Flow, pressure, and transparent corridor risk.</h2>
      <p class="dark-section-intro">Corridor Risk Score (CRS) summarizes structural remittance-flow stress from validated data. Higher scores indicate elevated stress — not a price forecast.</p>

      <div class="insight-strip" id="insightStrip">
        <article class="insight-card crs-card" data-animate>
          <div class="insight-label">Corridor Risk Score</div>
          <div class="insight-value neutral" id="insightCrs">{html.escape(str(crs_score))}</div>
          <div class="insight-sub">{html.escape(str(crs_band))} · 0–100 · {html.escape(data_label)}</div>
        </article>
        <article class="insight-card" data-animate>
          <div class="insight-label">Latest monthly flow</div>
          <div class="insight-value neutral" id="insightLatest" data-count="{latest}" data-suffix="">0</div>
          <div class="insight-sub">{html.escape(kpis.get('latest_month', 'Latest'))} · USD millions · {html.escape(data_label)}</div>
        </article>
        <article class="insight-card" data-animate>
          <div class="insight-label">Latest YoY change</div>
          <div class="insight-value {yoy_class}" id="insightYoyPct" data-count="{yoy_pct}" data-decimals="1" data-suffix="%">0</div>
          <div class="insight-delta {yoy_class}" id="insightYoyAbs">{html.escape(yoy_abs_text)}</div>
          <div class="insight-sub">vs same month prior year · {html.escape(data_label)}</div>
        </article>
        <article class="insight-card" data-animate>
          <div class="insight-label">2025 YTD flow</div>
          <div class="insight-value neutral" id="insightYtd" data-count="{ytd}" data-suffix="">0</div>
          <div class="insight-sub">Jan–Nov 2025 · USD millions · {html.escape(data_label)}</div>
        </article>
        <article class="insight-card" data-animate>
          <div class="insight-label">Peak month</div>
          <div class="insight-value peak" id="insightPeak" data-count="{peak}" data-suffix="">0</div>
          <div class="insight-sub">{html.escape(kpis.get('peak_month', ''))} · dataset high · {html.escape(data_label)}</div>
        </article>
      </div>

      <div class="story-panel" id="storyPanel" data-animate>
        <div class="story-kicker">Corridor read · {html.escape(data_label)}</div>
      ...
      <div class="corridor-methodology">
        ...
        <h4>Methodology</h4>
        <p>Corridor Risk Score combines momentum, volatility, drawdown-from-peak, and data-quality components from validated monthly remittance data. See <code>model-lab/docs/corridor_intelligence_framework.md</code>. Research communication only — not trading signals or FX forecasts.</p>
        {brief_link}
      </div>
    </div>
  </section>
...
def bfi_us_mexico_corridor_scripts() -> str:
    payload_json = json.dumps(_corridor_payload() or {})
    chart_js_path = ROOT / "src" / "corridor_intelligence" / "dashboard_charts.js"
    chart_body = chart_js_path.read_text(encoding="utf-8")
    return (
        '<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>\n<script>\n(function () {\n'
        f"  const CORRIDOR = {payload_json};\n"
        + chart_body
        + "\n})();\n</script>\n"
    )
...
<main class="bfi-main">
  ...
  <section class="bfi-section">
    <h2>Structure</h2>
    <p><strong>Bowers Frontier Macro Labs</strong> — FX, rates, corridors, macro structure, and integrated data lake.<br/>
    ...
    <strong>Bowers Frontier Studio</strong> — Visual systems, dashboards, publication design.</p>
  </section>
  <section class="bfi-section">
